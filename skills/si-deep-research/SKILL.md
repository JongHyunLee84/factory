...
### Full Mode (Default)
When invoked directly via `/si-deep-research` or without topic parameters.
Executes the complete workflow: Step 0 (Scope Definition) → Step 1-4.

### Focused-Topic Mode
When called by `si-1-research` with a specific topic, focus questions, and output path.
**Entry condition**: caller provides `topic`, `focus_questions`, and `output_path`.

In Focused-Topic Mode:
- **Skip** Step 0 (Scope Definition) — scope is already defined by the caller
