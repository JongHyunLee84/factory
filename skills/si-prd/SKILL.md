...
사전 자료 읽기:
- `tasks/si-1-research.md` (Research phase 산출물, 있으면)
- `tasks/si-1-research/` (topic sub-reports, 있으면)
...
Phase 0 분석 후, 프로젝트 복잡도를 분류하여 인터뷰 깊이를 조절한다.

```
AskUserQuestion(questions=[
  {
    question: "이 프로젝트의 규모와 복잡도는 어느 정도인가요?",
    header: "복잡도",
    multiSelect: false,
    options: [
      { label: "Simple", description: "단일 기능, 1-2 파일 변경 수준" },
      { label: "Medium", description: "모듈 단위, 3-5 파일에 걸친 변경" },
      { label: "Complex", description: "시스템 단위, 6+ 파일, 여러 컴포넌트 연동" }
    ]
  }
])
```
...
**복잡도별 인터뷰 깊이:**
- **Simple**: Core 6 질문 + 후속 1라운드 → Phase 2로 진행
- **Medium**: Core 6 + 목적 심화 + 시나리오 워크스루 + 부정 시나리오 탐색
- **Complex**: Core 6 + 목적 심화 + 시나리오 워크스루 + 부정 시나리오 탐색 + 트레이드오프 매트릭스

이 분류 결과를 이후 Phase 1 전체에서 참조한다.
...
### "5 Whys" Purpose Deepening (Medium/Complex)
...
### Scenario Walkthrough (Medium/Complex)
...
### Negative Scenario Probing (Medium/Complex)
...
- [ ] 주요 사용자 시나리오가 1개 이상 서술되었는가? (Medium/Complex)
- [ ] 비목표(Non-goals)가 명시되었는가?
- [ ] 성공 기준에 수치가 포함되었는가?
- [ ] 실패/에러 케이스가 논의되었는가? (Medium/Complex)
- [ ] 기존 제약사항이 확인되었는가?
...
확정된 브리프를 바탕으로 PRD를 작성한다. `tasks/si-2-prd.md`에 출력.
...
| 복잡도 적응 | 프로젝트 규모에 맞게 인터뷰 깊이 조절 |

---

## Output

- `tasks/si-2-prd.md` — final PRD
...
- **복잡도 무시**: Simple 프로젝트에 과도한 인터뷰, 또는 Complex에 얕은 탐색
