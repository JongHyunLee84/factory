...
이 스킬은 워크플로우의 첫 번째 스킬이므로, 이전 decisions 파일이 없는 것이 일반적이다. 기존 `factory/*/decisions.md`가 있으면 읽고 참고한다.
...
| ID | Decision | 비위임 |
|----|----------|--------|
| R-D-001 | **리서치 토픽 선정/승인** — 사용자가 토픽 추가/제거/수정 | Yes |
| R-D-002 | 토픽 분해 승인 (Multi Topic Mode) | No |
| R-D-003 | URL 20+개 시 선별 기준 | No |

**Tier 2 (Tactical — AI 기본값 + 사용자 오버라이드):**

| ID | Decision | AI 기본값 |
|----|----------|----------|
| R-D-004 | 종합 방식 (토픽별 vs 비교 분석) | 토픽별 정리 |
| R-D-005 | 차단 시 범위 축소 방향 | 우선순위 기반 축소 (API Ref > Getting Started > Examples) |

### 0-3. Agenda Overview
`AskUserQuestion`으로 진행 방식 확인:
- **핵심만 검토 (Recommended)**: Tier 1만 직접, Tier 2는 AI 기본값
- **전체 검토**: Tier 1 + Tier 2 모든 결정을 직접
- **알아서 해줘**: AI가 결정하고 근거 기록. R-D-001(리서치 토픽 선정)만 반드시 확인

### 0-4. Decision Walk-through
선택에 따라 `AskUserQuestion`으로 결정 수집 (최대 4개/배치, 의존성 순서 준수).
- R-D-001, R-D-002는 Multi Topic Mode의 Step B, C에서 자연스럽게 진행된다.
- R-D-003은 Step 1에서 URL이 20개를 초과할 때 트리거된다.

### 0-5. Record Decisions
`factory/research/decisions.md`에 기록:

| ID | Decision | Choice | Rationale | Decided By | Date |
|----|----------|--------|-----------|------------|------|

### 0-6. Proceed
모든 선행 결정 기록 후 Single Topic Mode 또는 Multi Topic Mode로 진행.
...
- `factory/research/decisions.md` — 의사결정 기록
