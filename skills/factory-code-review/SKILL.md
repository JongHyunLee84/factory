...
- **알아서 해줘**: AI가 결정하고 근거 기록. CR-D-002(판정 기준 오버라이드)만 반드시 확인

### 0-4. Decision Walk-through
선택에 따라 `AskUserQuestion`으로 결정 수집 (최대 4개/배치, 의존성 순서 준수).
- CR-D-001은 Step 4 Issue Review 후 심각도 분류 결과를 사용자에게 확인한다.
- CR-D-002는 Step 3 Compliance Score 산출 전에 기준을 사용자에게 확인한다.

### 0-5. Record Decisions
`factory/code-review/decisions.md`에 기록:
...
결정 기록은 `factory/code-review/decisions.md`에 저장되었습니다.
